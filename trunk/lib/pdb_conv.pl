...
$USAGE = "
 ...
 This command reads Protein Data Bank file PDB_INPUT and writes
 XPLOR/CHARMM PDB file PDB_OUTPUT.
 ...
 -m CHAIN_ID -- write only atoms with the chain ID equal
 to CHAIN_ID
...
# requirements: None
# body
if ( ($opt_f_value ne "charmm") && ($opt_f_value ne "xplor") ) {
  error( "output PDB format must be either 'charmm' or 'xplor'" );
}
...
while ( <$file_i> ) {
  ...
  if ( defined($fields[0]) &&
    ...
    if ( $alt_loc_ok && $chain_ok && $hydro_ok && $hetatm_ok ) {

      $atom_cntr++;

      if ( $opt_f_value eq "charmm" ) {
...
if ( $atom_cntr > 0 ) {
  printf " %d atoms written to the file '$output_file'\n", $atom_cntr;
} else {
